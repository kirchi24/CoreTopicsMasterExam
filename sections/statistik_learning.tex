...
\textbf{Wahrscheinlichkeitsfkt.} & PMF: $f(x_i) = P(X = x_i) \geq 0$ \newline $\sum_i f(x_i) = 1$ & PDF: $f(x) \geq 0$, $\int_{-\infty}^{\infty} f(x)\,dx = 1$ \newline $P(X = x) = 0$ für jeden Einzelwert \\[8pt]
\textbf{Intervallwahrsch.} & $P(X \in A) = \sum_{x_i \in A} p_i$ & $P(a < X \leq b) = \int_a^b f(x)\,dx = F(b)-F(a)$ \\[4pt]
...
\definition{\textbf{Bayesianische Inferenz:} Ein unbekannter Parameter $\theta$ wird als Zufallsvariable behandelt. Der Satz von Bayes aktualisiert das Wissen über $\theta$, sobald Daten $\mathcal D$ beobachtet werden:
...
\definition{\textbf{Punktschätzer \& Unsicherheit:} Aus dem Posterior (einer Verteilung) leitet man ab:
\begin{itemize}[leftmargin=1.5em, noitemsep]
    \item \textbf{MAP}: $\hat\theta_{\text{MAP}} = \arg\max_\theta p(\theta\mid\mathcal D)$ (Modus)
    \item \textbf{Posterior Mean}: $\mathbb E[\theta\mid\mathcal D]$
...
\textbf{Beispiel -- Münzwurf:} Prior $\text{Beta}(1,1)$.
\begin{center}
\begin{tabular}{@{}lccc@{}}
\toprule
...
\end{tabular}
\end{center}
Gesamt ($n{=}9,k{=}6$) auf den Ausgangsprior angewendet ergibt ebenfalls $\text{Beta}(7,4)$ -- bestätigt das Sequential Update. Zum Vergleich: MLE wäre $\hat p=0{,}6$; der Bayes-Schätzer wird leicht zu $0{,}5$ gezogen.
% ============================================================
\newpage
