...
Der Satz erlaubt, ``rückwärts'' zu schließen: von bekanntem $P(B|A)$ auf gesuchtes $P(A|B)$. Die vier Terme: }
\begin{itemize}
...
\textbf{Beispiel -- Münzwurf:} Prior $p \sim \text{Beta}(1,1)$, Daten: $n=5$ Würfe mit $k=3$ Erfolgen. Aus der Likelihood $p(\mathcal D\mid p)\propto p^3(1-p)^2$ folgt der Posterior
$$p\mid\mathcal D \sim \text{Beta}(4,3).$$
Der Posterior Mean ist $\mathbb E[p\mid\mathcal D]=4/7\approx0.57$, während der Maximum-Likelihood-Schätzer (MLE) $\hat p_{\text{MLE}}=3/5=0.6$ ergibt. Der Bayes-Schätzer wird durch den Prior leicht in Richtung $0.5$ gezogen.
% ============================================================
\newpage
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{assets/bias_var_tradeoff.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{assets/bias_var_sum.png}
\end{figure}
...
Problem: direktes Sampling aus $p$ ist oft unmöglich (unbekannte Normalisierungskonstante).}
...
Bedingung: $q(x)>0$ überall wo $p(x)>0$. \textbf{Problem:} Schlechte Proposal $\Rightarrow$ wenige Samples dominieren (\emph{Weight Degeneracy}). \textbf{Effective Sample Size} $\mathrm{ESS} = {(\sum w_i)^2}/{\sum w_i^2} \in [1, n]$ misst Effizienz. In hohen Dimensionen versagt IS zuverlässig.}

\definition{\textbf{Markov Chain Monte Carlo (MCMC):} Konstruiere eine Markov Chain mit stationärer Verteilung $p(x)$; nach einer \emph{Burn-in}-Phase liefern die Zustände approximative Samples aus $p$ -- ohne die Normalisierungskonstante zu benötigen.
...
(3) sonst behalte $x_t$. Die \emph{Detailed Balance}-Bedingung garantiert $p$ als stationäre Verteilung.}
...
d.\,h. jede Variable wird aus ihrer \textbf{vollständig bedingten Verteilung} gezogen, während alle anderen auf ihrem aktuellen Wert fixiert bleiben.}
...
\begin{itemize}[leftmargin=1.5em,noitemsep]
    ...
    \item \textbf{Nachteil:} Langsames Mixing bei stark korrelierten Variablen (Chain ``hängt fest'').
