...
\bottomrule
\end{tabular}
\caption*{Übersicht: zulässige Maße je Skalenniveau (jede Zelle gilt zusätzlich zu den Maßen links davon, außer wo gesondert angegeben)}
\end{table}
...
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Fehler I.\ Art} ($\alpha$): $H_0$ abgelehnt, obwohl wahr -- \emph{Fehlalarm}. Kontrolliert durch die Wahl von $\alpha$ (üblich: $0{,}05$).
    \item \textbf{Fehler II.\ Art} ($\beta$): $H_0$ nicht abgelehnt, obwohl $H_1$ wahr -- \emph{falsch negativ}. Hängt von Effektgröße und $n$ ab.
...
\begin{enumerate}[leftmargin=1.5em]
    \item \textbf{Hypothesen:} $H_0: \mu = 50$ vs. $H_1: \mu \neq 50$ (zweiseitig); $\alpha = 0{,}05$.

    \item \textbf{Teststatistik:}
    $t_{\text{obs}} = \dfrac{\bar{x} - \mu_0}{s/\sqrt{n}} = \dfrac{50{,}8 - 50}{1{,}5/\sqrt{20}} \approx 2{,}39$

    \item \textbf{Kritischer Bereich:} $t_{19;\,0{,}025} \approx 2{,}093$ (zweiseitig, $df=19$). Ablehnungsbereich: $|t_{\text{obs}}| > 2{,}093$.

    \item \textbf{Entscheidung:} $2{,}39 > 2{,}093$ $\Rightarrow$ $H_0$ ablehnen; p-Wert $\approx 0{,}027 < 0{,}05$.
\end{enumerate}
...
\textbf{Annahmen:}
\begin{itemize}[leftmargin=1.5em]
    \item Linearität: $\mathbb{E}[Y|X=x] = \beta_0 + \beta_1 x$
    \item Unabhängigkeit der Fehler
    \item Homoskedastizität (konstante Fehlervarianz)
    \item Normalverteilung der Fehler (nur für Inferenz/Tests)
\end{itemize}
}
...
\textbf{Varianzzerlegung:}
$$ \underbrace{\text{TSS}}_{\text{Total Sum of Squares}} = \underbrace{\text{ESS}}_{\text{Explained Sum of Squares}} + \underbrace{\text{RSS}}_{\text{Residual Sum of Squares}} $$ d.\,h. $$\sum(y_i-\bar{y})^2 = \sum(\hat{y}_i-\bar{y})^2 + \sum e_i^2.$$

\textbf{Bestimmtheitsmaß:}
$$R^2 = \frac{\text{ESS}}{\text{TSS}} = 1 - \frac{\text{RSS}}{\text{TSS}} \in [0,1].$$
Anteil der durch das Modell erklärten Variabilität. Für SLR: $R^2 = r_{xy}^2$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{assets/dendogram.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{assets/bed_unabhängigkeit.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{assets/roc_curve.png}
\end{figure}
% ============================================================
\newpage
...
\definition{\textbf{Stationarität (schwach):} Konstanter Erwartungswert $E(Y_t)=\mu$, konstante Varianz $\operatorname{Var}(Y_t)=\sigma^2$, und Autokovarianz $\operatorname{Cov}(Y_t,Y_{t+h})$ hängt nur vom Lag $h$ ab. Voraussetzung für ARMA-Modelle; Prüfung via ACF.}

\definition{\textbf{ACF \& PACF:} Empirische Autokorrelation zum Lag $\omega$:
$$r_\omega = \frac{\sum_{t=\omega+1}^{T}(y_{t-\omega}-\bar{y})(y_t-\bar{y})}{\sum_{t=1}^{T}(y_t-\bar{y})^2}, \quad -1 \le r_\omega \le 1$$
Langsam abfallende ACF $\Rightarrow$ nicht-stationär; saisonale Spitzen $\Rightarrow$ periodische Struktur. \textbf{PACF} misst Korrelation bei Lag $\omega$ nach Herauspartialisieren aller dazwischenliegenden Lags -- Faustregel: PACF bricht bei Lag $p$ ab $\Rightarrow$ AR($p$); ACF bricht bei Lag $q$ ab $\Rightarrow$ MA($q$).}
...
\definition{\textbf{AR, MA, ARMA} ($\varepsilon_t \sim \text{WN}$):
\begin{itemize}[leftmargin=1.5em,noitemsep]
    ...
    \item \textbf{ARMA($p,q$):} Kombination; \textbf{ARIMA($p,d,q$):} $d$-faches Differenzieren erzeugt Stationarität
    \item \textbf{SARIMA$(p,d,q)\times(P,D,Q)_s$:} zusätzliche saisonale AR/MA-Terme mit Periode $s$
\end{itemize}}

\beispiel{\textbf{Modellierung in R:}\\
\texttt{arima(x, order=c(p,d,q), seasonal=list(order=c(P,D,Q), period=s))}\\
Modellwahl via AIC/BIC; Diagnose: Residuen $\overset{!}{\sim}$ WN (Ljung-Box-Test, ACF $\approx 0$).}
