...
\textbf{Multivariate Regression:} In der Statistik bedeutet ``multivariat'' typischerweise \textbf{mehrere Response-Variablen} $y_1, \ldots, y_k$ (nicht mehrere Prädiktoren!). Obwohl Regression meist mit \textit{einer} Response erfolgt, kann man auch $k$ separate (oder gemeinsame) Regressionen fittig:
...
Beliebte Wahlten: B-Splines, natural cubic splines, smooth functions. Mit dieser Darstellung wird das GAM-Optimization wieder zu einem \textbf{regularisierten Linearen Problem}.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \end{itemize}
    \item \textbf{Non-Metric MDS:} Versucht, nur die \textbf{Ordnung} der Distanzen zu erhält (kleinste Distanz bleibt klein, größte bleibt groß, aber exakte Werte sind unwichtig).
    \begin{itemize}
...
\definition{\textbf{Regularisierung und Shrinkage-Methoden:} Techniken, die die Größe der Regressionskoeffizienten beschränken (reduzieren), um Overfitting zu vermeiden und stabilere Schätzungen zu erhalten. Dies ist besonders wichtig bei hohdimensionalen Daten oder Multikollinearität.}
...
\textbf{Variablen-Auswahl-Effekt:} Wenn Lasso Koeffizienten auf exakt Null setzt, diese Variablen aus dem Modell entfernt. Dies ist besonders wertvoll wenn:
\begin{itemize}[leftmargin=1.5em]
...
\begin{enumerate}[leftmargin=2em]
    ...
    \begin{itemize}
        ...
        \item Trainiere einen vollständigen (unkürzen) Decision Tree auf dieser Stichprobe.
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item Vorhersage: Gemittel (oder Mehrheits-Stimme) dieser Bäume für diese Beobachtung.
...
\definition{\textbf{Parametereschätzung in LDA:}
...
\definition{\textbf{Parametereschätzung in QDA:}
...
Summe der absoluten Differenzen — wie man sich in einem Stadtgitter bewegen würde (auch \textbf{Taxicab} oder \textbf{Chebyshev}-Distanz ähnlich).}
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{Sensibel gegenüber Skalierung:} Variablen mit großerer Streuung dominieren die Distanzberechnung — \textbf{Normalisierung ist wesentlich}.
    \item \textbf{Ausreißer-Problem:} Ein einzelner Ausreißer nah bei einem Test-Punkt kann Vorhersage verderben.
    \item \textbf{High Curse of Dimensionality:} In hochdimensionalen Räumen (großes p) werden Distanzen bedeutungslos; fast alle Punkte sind gleich weit entfernt. kNN funktioniert schlecht bei $p \gg n$.
    \item \textbf{Wahl von k unklar:} Keine offensichtliche Strategie, muss via CV tuned werden.
